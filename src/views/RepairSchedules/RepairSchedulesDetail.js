...
const RepairScheduleDetail = () => {
  ...
  // Check if repairSchedule is provided, if not show a message
  if (!repairSchedule) {
  ...
  const handleBack = () => {
    navigate(-1) // Quay lại màn hình trước đó
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate('/repairSchedules') // Quay về màn hình danh sách lịch sửa chữa
      } else {
            ...
            <CRow className="mb-3">
              <CCol className="text-start">
                ...
                <div
                  style={{
                    display: 'flex',
                    justifyContent: 'flex-start', // Đặt hình ảnh sang trái
                    flexWrap: 'wrap',
                    marginTop: '10px',
                  }}
                >
