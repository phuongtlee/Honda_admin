...
const StarRating = ({ rating }) => {
  ...
  // Generate an array with full and empty stars based on the rating
  const stars = Array(5)
    .fill(emptyStar)
...
const Reviews = () => {
  const [reviewsData, setReviewsData] = useState([]) // Store review data
  const [filteredReviews, setFilteredReviews] = useState([]) // Filtered review data by month
  const [currentPage, setCurrentPage] = useState(1)
  const [totalPages, setTotalPages] = useState(1)
  const [error, setError] = useState(null)
  const [searchQuery, setSearchQuery] = useState('')
  const [showAlert, setShowAlert] = useState(true)
  const [selectedMonth, setSelectedMonth] = useState('') // Store selected month for filtering
  ...
  useEffect(() => {
    ...
    if (selectedMonth) {
      filtered = filtered.filter((review) => {
        ...
        const reviewMonth = reviewDate.getMonth() + 1 // Get month (1-based index)
        return reviewMonth === parseInt(selectedMonth, 10)
      })
    }

    // Sort filtered reviews by createdAt (descending order)
  ...
  const handleMonthChange = (e) => {
    setSelectedMonth(e.target.value) // Set selected month for filtering
  }
